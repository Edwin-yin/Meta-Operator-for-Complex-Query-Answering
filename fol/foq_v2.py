...
class FirstOrderSetQuery(ABC):
    ...
    @abstractmethod
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None,
                             freeze_dumps: Dobject = None, meta_parameters=None):
        """
        full_distance_config is a dict which contains three parameter: use_distance, distance_operators, distance2index
        The use_distance is in [False, root, leaf] and different MLP will assign to a operator with different distance.
        distance2index helps to get the final net_index
        freeze_dumps is a json Dobject to give instruction on freezing certain parameters.
        meta_parameters is a dict which allows for outside parameters used in training for meta learning.
        """
        pass
    ...
    def _embedding_optimization(self, estimator: AppFOQEstimator,
        ...
        it first initializes the latent embeddings of each fosq instance
    ...
    @abstractmethod
    def get_leaf_distance(self, count_list):
        """
        This returns a dict for telling how many operators it have in the query tree, also let all operator records its
        own leaf_distance.
        By leaf_distance, it means that anchor node has a depth of 0.
        It will return final_distance, final_accumulative, the latter is a nested dict, which keys are operator name and
        depth, then return the num of operator in the tree.
        Example: {'p': {'1: 2, 2: 1}}
        """
        pass

    @abstractmethod
    def get_root_distance(self, count_list, now_distance):
        """
        This returns a dict for telling how many operators it have in the query tree, also let all operator records its
        own leaf_distance.
        By root_distance, it means that the root(query point) has a distance of 0.
        """
        pass

    @abstractmethod
    def assign_output_symbol(self, previous_symbol, count_list):
        """
        Assign net_symbol for every operator according to their output.
        previous_symbol should be 'a' initially, 'a' stands for answer.
        """
        pass

    @abstractmethod
    def assign_input_symbol(self, count_list):
        pass

    @abstractmethod
    def assign_net_symbol(self, distance_method, count_list, previous_symbol):
        """
        Assign net_symbol for every operator according to their output/input/output binary/input binary.
        This is an integrated function.
        previous_symbol should be 'a' initially, 'a' stands for answer.
        """

    @abstractmethod
    def freeze_distance_dumps(self, operator_dump_dict: dict):
        """
        operator_dump_dict is a dict that keys are operator name and values are distances that need to be frozen
        like (p,(p,(e))) and {'p': [1]} with root will get a
        {"o": "p", "a":[["False", "False"], {"o": "p", "a": [["False", "True"], {"o": "e", "a": ["False"]}]}]}
        (p,(p,(e))) and {'p': [1], 'e':[], 'r':[]} will lead to
        {"o": "p", "a":[["True", "False"], {"o": "p", "a": [["True", "True"], {"o": "e", "a": ["True"]}]}]}
        """
        pass

    @abstractmethod
    def assign_net_index(self, operator_distance2index, full_distance_config=None):
        """
        -1 means not using distance now
        operator_distance2index is like {'p': {2:0, 5:1, 7:2}}
        """
        pass

    @abstractmethod
    def clear_net_index(self):
        """
        Due to the fact that assign_fosq_distance doesn't change 0/1 existing index back to -1, I create a fucntion
        to do that.
        """
        pass
...
def get_net_index(fosq: FirstOrderSetQuery, full_distance_config=None):
    """
    -1 means not using distance now
    """
    net_index = -1
    if not full_distance_config or not full_distance_config['use_distance']:
        return net_index
    if fosq.__o__ in full_distance_config['distance_operators']:
        if full_distance_config['use_distance'] == 'leaf':
            net_index = full_distance_config['distance2index'][fosq.__o__][fosq.leaf_distance]
        elif full_distance_config['use_distance'] == 'root':
            net_index = full_distance_config['distance2index'][fosq.__o__][fosq.root_distance]
    return net_index



def get_distance(fosq: FirstOrderSetQuery, use_distance):
    if use_distance == 'leaf':
        return fosq.leaf_distance
    elif use_distance == 'root':
        return fosq.root_distance

def assign_fosq_distance(fosq, operator_distance2index, full_distance_config):
    """
    Assign net_index for a single operator.
    """
    net_index = -1
    if not full_distance_config or not full_distance_config['use_distance']:
        return net_index
    if fosq.__o__ in operator_distance2index:
        if full_distance_config['use_distance'] == 'leaf':
            net_index = operator_distance2index[fosq.__o__][fosq.leaf_distance]
        elif full_distance_config['use_distance'] == 'root':
            net_index = operator_distance2index[fosq.__o__][fosq.root_distance]
    if net_index == -1 and fosq.net_symbol != -1:
        net_index = fosq.net_symbol # In this case, don't change net_index back to -1.
    return net_index
...
class Entity(FirstOrderSetQuery):
    ...
    def __init__(self):
        ...
        self.tentities = None
        self.device = 'cpu'
        self.root_distance , self.leaf_distance, self.net_symbol = 0, 0, -1
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None, freeze_dumps=None,
        ...
        now_freeze = False
        if freeze_dumps:
            assert freeze_dumps['o'] == self.__o__
            now_freeze = freeze_dumps['a'][0]
        return estimator.get_entity_embedding(ent, now_freeze, meta_parameters)

    def _embedding_optimization(self,
                                estimator: AppFOQEstimator,
                                batch_indices=None,
                                **kwargs):
        self.latent_embedding = self.embedding_estimation(estimator=estimator, batch_indices=batch_indices)
        return None, [],
    ...
    def get_leaf_distance(self, count_list):
        now_distance = 0
        accumulate_distance = fixed_depth_nested_dict(int, 2)
        accumulate_distance['e'][0] = 1
        self.leaf_distance = now_distance
        return now_distance, accumulate_distance

    def get_root_distance(self, count_list, now_distance):
        if 'e' in count_list:
            now_distance += 1
        accumulative = fixed_depth_nested_dict(int, 2)
        accumulative['e'][now_distance] = 1
        self.root_distance = now_distance
        return accumulative

    def assign_output_symbol(self, previous_symbol, count_list):
        accumulative = fixed_depth_nested_dict(int, 2)
        if 'e' in count_list:
            self.net_symbol = previous_symbol
            accumulative['e'][self.net_symbol] = 1
        return accumulative
    ...
    def assign_net_symbol(self, distance_method, count_list, previous_symbol):
        accumulative = fixed_depth_nested_dict(int, 2)
        if 'e' in count_list:
            if distance_method == 'output':
                self.net_symbol = previous_symbol
                accumulative['e'][self.net_symbol] = 1
            elif distance_method == 'output_binary':
                self.net_symbol = 'a' if previous_symbol == 'a' else 'na'
            elif distance_method == 'input':
                pass
            elif distance_method == 'input_binary':
                pass
            accumulative['e'][self.net_symbol] = 1
        return accumulative
    ...
    def freeze_distance_dumps(self, operator_dump_dict: dict):
        ...
        if self.__o__ in operator_dump_dict and self.net_symbol in operator_dump_dict[self.__o__]:
            dobject['a'] = [True]
        return json.dumps(dobject)

    def assign_net_index(self, operator_distance2index, full_distance_config=None):
        self.net_symbol = assign_fosq_distance(self, operator_distance2index, full_distance_config)
        return self.net_symbol
...
class Negation(FirstOrderSetQuery):
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None,
                             freeze_dumps=None, meta_parameters=None):
        if freeze_dumps:
            assert freeze_dumps['o'] == self.__o__
        now_freeze = freeze_dumps['a'][0] if freeze_dumps else False
        operand_freeze = freeze_dumps['a'][1] if freeze_dumps else None
        operand_emb = self.query.embedding_estimation(estimator, batch_indices, full_distance_config, operand_freeze,
                                                      meta_parameters)
        return estimator.get_negation_embedding(operand_emb, meta_parameters)
    ...
    def get_leaf_distance(self, count_list):
        previous_distance, accumulative = self.query.get_leaf_distance(count_list)
        if count_list:
            now_distance = previous_distance + 1
        else:
            now_distance = previous_distance
        accumulative['n'][now_distance] += 1
        self.leaf_distance = now_distance
        return now_distance, accumulative

    def get_root_distance(self, count_list, now_distance):
        if 'n' in count_list:
            now_distance += 1
        sub_accumulative = self.query.get_root_distance(count_list, now_distance)
        sub_accumulative['n'][now_distance] += 1
        self.root_distance = now_distance
        return sub_accumulative

    def assign_output_symbol(self, previous_symbol, count_list):
        sub_accumulative = self.query.assign_output_symbol('n', count_list)
        if 'n' in count_list:
            self.net_symbol = previous_symbol
            sub_accumulative['n'][self.net_symbol] += 1
        return sub_accumulative

    def assign_input_symbol(self, count_list):
        sub_accumulative = self.query.assign_input_symbol(count_list)
        if 'n' in count_list:
            self.net_symbol = self.query.__o__
            sub_accumulative['n'][self.net_symbol] += 1
        return sub_accumulative

    def assign_net_symbol(self, distance_method, count_list, previous_symbol):
        sub_accumulative = self.query.assign_net_symbol(distance_method, count_list, 'n')
        if 'n' in count_list:
            if distance_method == 'output':
                self.net_symbol = previous_symbol
            elif distance_method == 'output_binary':
                ...
                self.net_symbol = 'e' if self.query.__o__ == 'e' else 'ne'
            sub_accumulative['n'][self.net_symbol] += 1
        return sub_accumulative
    ...
    def freeze_distance_dumps(self, operator_dump_dict: dict):
        dobject = {
            'o': self.__o__,
            'a': [False, json.loads(self.query.freeze_distance_dumps(operator_dump_dict))]
        }
        if self.__o__ in operator_dump_dict and self.net_symbol in operator_dump_dict[self.__o__]:
            dobject['a'][0] = True
        return json.dumps(dobject)
    ...
    def assign_net_index(self, operator_distance2index, full_distance_config=None):
        self.net_symbol = assign_fosq_distance(self, operator_distance2index, full_distance_config)
        self.query.assign_net_index(operator_distance2index, full_distance_config)
        return self.net_symbol

    def clear_net_index(self):
        self.net_symbol = -1
        self.query.clear_net_index()
...
class Projection(FirstOrderSetQuery):
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None,
        ...
        if batch_indices:
            ...
            rel = self.trelations
        now_freeze_rel, now_freeze_proj, operand_freeze = False, False, None
        if freeze_dumps:
            assert self.__o__ == freeze_dumps['o']
            now_freeze, operand_freeze = freeze_dumps['a']
            now_freeze_rel, now_freeze_proj = now_freeze
        operand_emb = self.query.embedding_estimation(estimator, batch_indices, full_distance_config, operand_freeze,
                                                      meta_parameters)
        return estimator.get_projection_embedding(rel, operand_emb, self.net_symbol if full_distance_config else -1,
                                                  now_freeze_rel, now_freeze_proj, meta_parameters)
    ...
    def get_leaf_distance(self, count_list):
        previous_distance, accumulative = self.query.get_leaf_distance(count_list)
        now_distance = previous_distance + 1 if 'p' in count_list else previous_distance
        accumulative['p'][now_distance] += 1
        self.leaf_distance = now_distance
        return now_distance, accumulative

    def get_root_distance(self, count_list, now_distance):
        next_distance = now_distance + 1 if 'p' in count_list else now_distance
        sub_accumulative = self.query.get_root_distance(count_list, next_distance)
        sub_accumulative['p'][now_distance] += 1
        self.root_distance = now_distance
        return sub_accumulative

    def assign_output_symbol(self, previous_symbol, count_list):
        sub_accumulative = self.query.assign_output_symbol('p', count_list)
        if 'p' in count_list:
            self.net_symbol = previous_symbol
            sub_accumulative['p'][self.net_symbol] += 1
        return sub_accumulative
    ...
    def assign_net_symbol(self, distance_method, count_list, previous_symbol):
        sub_accumulative = self.query.assign_net_symbol(distance_method, count_list, 'p')
        if 'p' in count_list:
            if distance_method == 'output':
                self.net_symbol = previous_symbol
            elif distance_method == 'output_binary':
    ...
    def freeze_distance_dumps(self, operator_dump_dict: dict):
        dobject = {
            'o': self.__o__,
            'a': [[False, False], json.loads(self.query.freeze_distance_dumps(operator_dump_dict))]
        }
        if self.__o__ in operator_dump_dict and self.net_symbol in operator_dump_dict[self.__o__]:
            dobject['a'][0][1] = True
        if 'r' in operator_dump_dict:
            dobject['a'][0][0] = True
        return json.dumps(dobject)
...
class MultipleSetQuery(FirstOrderSetQuery):
    def __init__(self, *queries: List[FirstOrderSetQuery]):
        self.sub_queries: List[FirstOrderSetQuery] = queries
        self.root_distance, self.leaf_distance, self.net_symbol = 0, 0, -1
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None, freeze_dumps=None,
                             meta_parameters=None):
        """
        Get all sub_embedding in this function, aggregation is done in specific operand class.
        """
        if freeze_dumps:
            assert freeze_dumps['o'] == self.__o__
            return [
                self.sub_queries[i].embedding_estimation(estimator, batch_indices, full_distance_config,
                                                         freeze_dumps['a'][1][i], meta_parameters)
                for i in range(len(self.sub_queries))]
        else:
            return [q.embedding_estimation(estimator, batch_indices, full_distance_config, None, meta_parameters)
    ...
    def get_leaf_distance(self, count_list):
        now_distance, now_accumulative = 0, fixed_depth_nested_dict(int, 2)
        for sub_query in self.sub_queries:
            sub_distance, sub_accumulative = sub_query.get_leaf_distance(count_list)
            now_distance = max(now_distance, sub_distance)
            for operator_key in sub_accumulative:
                for length in sub_accumulative[operator_key]:
                    now_accumulative[operator_key][length] += sub_accumulative[operator_key][length]
        now_distance = now_distance + 1 if self.__o__ in count_list else now_distance
        now_accumulative[self.__o__][now_distance] += 1
        self.leaf_distance = now_distance
        return now_distance, now_accumulative

    def get_root_distance(self, count_list, now_distance):
        now_operator = self.__o__
        next_distance = now_distance + 1 if now_operator in count_list else now_distance
        now_cumulative = fixed_depth_nested_dict(int, 2)
        now_cumulative[now_operator][now_distance] = 1
        for sub_query in self.sub_queries:
            sub_cumulative = sub_query.get_root_distance(count_list, next_distance)
            for operator_key in sub_cumulative:
                for length in sub_cumulative[operator_key]:
                    now_cumulative[operator_key][length] += sub_cumulative[operator_key][length]
        self.root_distance = now_distance
        return  now_cumulative

    def assign_output_symbol(self, previous_symbol, count_list):
        now_accumulative = fixed_depth_nested_dict(int, 2)
        if self.__o__ in count_list:
            self.net_symbol = previous_symbol
            now_accumulative[self.__o__][self.net_symbol] += 1
        for sub_query in self.sub_queries:
            sub_accumulative = sub_query.assign_output_symbol(self.__o__, count_list)
            for operator in sub_accumulative:
                for operator_type in sub_accumulative[operator]:
                    now_accumulative[operator][operator_type] += sub_accumulative[operator][operator_type]
        return now_accumulative

    def assign_input_symbol(self, count_list):
        now_accumulative = fixed_depth_nested_dict(int, 2)
        for sub_query in self.sub_queries:
            sub_accumulative = sub_query.assign_input_symbol(count_list)
            for operator in sub_accumulative:
                for operator_type in sub_accumulative[operator]:
                    now_accumulative[operator][operator_type] += sub_accumulative[operator][operator_type]
        return now_accumulative
    ...
    def assign_net_symbol(self, distance_method, count_list, previous_symbol):
        ...
        if self.__o__ in count_list:
            if distance_method == 'output':
                ...
                assert False, "input of multiple set query don't make sense"
            elif distance_method == 'input_binary':
                assert False, "input of multiple set query don't make sense"
            now_accumulative[self.__o__][self.net_symbol] += 1
        for sub_query in self.sub_queries:
            sub_accumulative = sub_query.assign_net_symbol(distance_method, count_list, self.__o__)
            for operator in sub_accumulative:
                for operator_type in sub_accumulative[operator]:
                    now_accumulative[operator][operator_type] += sub_accumulative[operator][operator_type]
        return now_accumulative
    ...
    def freeze_distance_dumps(self, operator_dump_dict: dict):
        ...
        dobject = {
            'o': self.__o__,
            'a': [False, [json.loads(subq.freeze_distance_dumps(operator_dump_dict))
    ...
    def assign_net_index(self, operator_distance2index, full_distance_config=None):
        ...
        self.net_symbol = assign_fosq_distance(self, operator_distance2index, full_distance_config)
        for sub_query in self.sub_queries:
            sub_query.assign_net_index(operator_distance2index, full_distance_config)
        return self.net_symbol

    def clear_net_index(self):
        self.net_symbol = -1
        for sub_query in self.sub_queries:
            sub_query.clear_net_index()
...
class Intersection(MultipleSetQuery):
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None, freeze_dumps=None,
                             meta_parameters=None):
        embed_list = super().embedding_estimation(estimator, batch_indices, full_distance_config, freeze_dumps,
                                                  meta_parameters)
        now_freeze = freeze_dumps['a'][0] if freeze_dumps else False
        if freeze_dumps:
            assert freeze_dumps['o'] == self.__o__
        return estimator.get_conjunction_embedding(embed_list, self.net_symbol if full_distance_config else -1,
                                                   now_freeze, meta_parameters)
...
class Union(MultipleSetQuery):
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None, freeze_dumps=None,
        ...
        embed_list = super().embedding_estimation(estimator, batch_indices, full_distance_config, freeze_dumps,
                                                  meta_parameters)
        now_freeze = freeze_dumps['a'][0] if freeze_dumps else None
        if freeze_dumps:
            assert freeze_dumps['o'] == self.__o__
        return estimator.get_disjunction_embedding(embed_list, full_distance_config, meta_parameters)
...
class Difference(FirstOrderSetQuery):
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None, freeze_dumps=None,
                             meta_parameters=None):
        lemb = self.lquery.embedding_estimation(estimator, batch_indices, full_distance_config, freeze_dumps,
                                                meta_parameters)
        remb = self.rquery.embedding_estimation(estimator, batch_indices, full_distance_config, freeze_dumps,
                                                meta_parameters)
        return estimator.get_difference_embedding(lemb, remb, meta_parameters)
    ...
    def get_leaf_distance(self, count_list):
        left_distance, left_accumulative = self.lquery.get_leaf_distance(count_list)
        right_distance, right_accumulative = self.rquery.get_leaf_distance(count_list)
        now_distance = max(left_distance, right_distance)
        now_distance = now_distance + 1 if 'd' in count_list else now_distance
        for operator_key in left_accumulative:
            for length in left_accumulative[operator_key]:
                right_accumulative[operator_key][length] += left_accumulative[operator_key][length]
        self.leaf_distance = now_distance
        return now_distance, right_accumulative

    def get_root_distance(self, count_list, now_distance):
        next_distance = now_distance + 1 if 'd' in count_list else now_distance
        left_accumulative = self.lquery.get_root_distance(count_list, next_distance)
        right_accumulative = self.rquery.get_root_distance(count_list, next_distance)
        for operator_key in left_accumulative:
            for length in left_accumulative[operator_key]:
                right_accumulative[operator_key][length] += left_accumulative[operator_key][length]
        self.root_distance = now_distance
        return right_accumulative

    def assign_output_symbol(self, previous_symbol, count_list):
        now_accumulative = fixed_depth_nested_dict(int, 2)
        l_accumulative = self.lquery.assign_output_symbol('d', count_list)
        r_accumulative = self.rquery.assign_output_symbol('d', count_list)
        for operator in l_accumulative:
            for operator_type in l_accumulative[operator]:
                now_accumulative[operator][operator_type] += l_accumulative[operator][operator_type]
        for operator in r_accumulative:
            for operator_type in r_accumulative[operator]:
                now_accumulative[operator][operator_type] += r_accumulative[operator][operator_type]
        if 'd' in count_list:
            self.net_symbol = previous_symbol
            now_accumulative['d'][self.net_symbol] += 1
        return now_accumulative

    def assign_input_symbol(self, count_list):
        now_accumulative = fixed_depth_nested_dict(int, 2)
        l_accumulative = self.lquery.assign_input_symbol(count_list)
        r_accumulative = self.rquery.assign_input_symbol(count_list)
        for operator in l_accumulative:
            for operator_type in l_accumulative[operator]:
                now_accumulative[operator][operator_type] += l_accumulative[operator][operator_type]
        for operator in r_accumulative:
    ...
    def assign_net_symbol(self, distance_method, count_list, previous_symbol=None):
        now_accumulative = fixed_depth_nested_dict(int, 2)
        l_accumulative = self.lquery.assign_net_symbol(distance_method, count_list, 'd')
        r_accumulative = self.rquery.assign_net_symbol(distance_method, count_list, 'd')
        for operator in l_accumulative:
            for operator_type in l_accumulative[operator]:
                now_accumulative[operator][operator_type] += l_accumulative[operator][operator_type]
        for operator in r_accumulative:
    ...
    def freeze_distance_dumps(self, operator_dump_dict: dict):
        dobject = {
            'o': self.__o__,
            'a': [False , [json.loads(self.lquery.freeze_distance_dumps(operator_dump_dict)),
                           json.loads(self.rquery.freeze_distance_dumps(operator_dump_dict))]]
        }
        if self.__o__ in operator_dump_dict and self.net_symbol in operator_dump_dict[self.__o__]:
            dobject['a'][0] = True
        return json.dumps(dobject)
    ...
    def assign_net_index(self, operator_distance2index, full_distance_config=None):
        self.net_symbol = assign_fosq_distance(self, operator_distance2index, full_distance_config)
        self.lquery.assign_net_index(operator_distance2index, full_distance_config)
        self.rquery.assign_net_index(operator_distance2index, full_distance_config)
        return self.net_symbol

    def clear_net_index(self):
        self.net_symbol = -1
        self.lquery.clear_net_index()
        self.rquery.clear_net_index()
...
class Multiple_Difference(MultipleSetQuery):
    ...
    def embedding_estimation(self, estimator: AppFOQEstimator, batch_indices, full_distance_config=None, freeze_dumps=None,
        ...
        embed_list = super().embedding_estimation(estimator, batch_indices, full_distance_config, freeze_dumps,
                                                  meta_parameters)
        return estimator.get_multiple_difference_embedding(embed_list, meta_parameters)
