...
class MetaLogicIntersection(MetaModule):
    ...
    def meta_forward(self, embeddings, meta_parameters=None):
        if self.use_att:  # use attention with weighted t-norm
            layer1_act = F.relu(self.layer1.meta_forward(
                embeddings, self.get_subdict(meta_parameters, 'center_net_0.layer1') if meta_parameters else None))
            # (num_conj, batch_size, 2 * dim)
            layer2_act = self.layer2.meta_forward(
                layer1_act, self.get_subdict(meta_parameters, 'center_net_0.layer2') if meta_parameters else None)
            attention = F.softmax(layer2_act, dim=0)
            # (num_conj, batch_size, dim)
            real_attention = attention / torch.max(attention, dim=0, keepdim=True).values

            if self.bounded:  # same weight for bound pair
                real_attention = torch.cat([real_attention, real_attention], dim=-1)

            if self.tnorm == 'mins':  # minimum / Godel t-norm
                smooth_param = -10  # smooth minimum
                min_weights = real_attention * torch.exp(smooth_param * embeddings)
                embedding = torch.sum(min_weights * embeddings, dim=0) / torch.sum(min_weights, dim=0)
                if self.bounded:
                    embedding = order_bounds(embedding)

            elif self.tnorm == 'luk':  # Lukasiewicz t-norm
                embedding = 1 - torch.sum(real_attention * (1 - embeddings), dim=0)
                if self.use_gtrans:
                    embedding = valclamp(embedding, b=6. / embedding.shape[0])
                else:
                    embedding = torch.clamp(embedding, 0, 1)
                ...
                embedding = torch.prod(torch.pow(torch.clamp(embeddings, 0, 1) + eps, real_attention), dim=0)
...
class MetaLogicProjection(MetaModule):
    ...
    def forward(self, e_embedding, r_embedding, freeze=False):
        x = torch.cat([e_embedding, r_embedding], dim=-1)
        if freeze:
            for nl in range(1, self.num_layers + 1):
                now_layer = getattr(self, "layer{}".format(nl))
                x = F.linear(x, now_layer.weight.data, now_layer.bias.data)
                x = F.relu(x)
            x = F.linear(x, self.layer0.weight.data, self.layer0.bias.data)
        else:
            for nl in range(1, self.num_layers + 1):
                x = F.relu(getattr(self, "layer{}".format(nl))(x))
            x = self.layer0(x)
    ...
    def meta_forward(self, e_embedding, r_embedding, meta_parameters=None):
        x = torch.cat([e_embedding, r_embedding], dim=-1)
        for nl in range(1, self.num_layers + 1):
            x = F.relu(getattr(self, "layer{}".format(nl)).meta_forward(
                x, params=self.get_subdict(meta_parameters, f'projection_net_0.layer{nl}')
                if meta_parameters else None))
        x = self.layer0.meta_forward(x,
                                     params=self.get_subdict(meta_parameters,
                                                             'projection_net_0.layer0') if meta_parameters else None)
        x = torch.sigmoid(x)
...
class LogicEstimator(AppFOQEstimator):
    def __init__(self, n_entity, n_relation, hidden_dim, gamma, entity_dim, relation_dim, num_layers,
                 negative_sample_size, t_norm, bounded, use_att, use_gtrans, device, projection_num=1,
                 conjunction_num=1):
        super().__init__()
        self.name = 'logic'
        self.device = device
        ...
        for index in range(projection_num):
            setattr(self, "projection_net_{}".format(index),
                    MetaLogicProjection(self.entity_dim * 2, self.relation_dim, hidden_dim, num_layers, bounded))
        for index in range(conjunction_num):
            setattr(self, "center_net_{}".format(index),
                    MetaLogicIntersection(self.entity_dim, t_norm, bounded, use_att, use_gtrans))

    def create_multiple_nets(self, projection_num, conjunction_num):
        """
        This approach should not be use since the newly appended module will not count as model.parameters and thus will
        not be optimized by optimizer.
        """
        for i in range(1, projection_num):
            self.projection_net_list.append(copy.deepcopy(self.projection_net_list[0]))
        for i in range(1, conjunction_num):
            self.center_net_list.append(copy.deepcopy(self.center_net_list[0]))
        for i in range(0, projection_num):
            self.projection_net_list[i].to(self.device)
        for j in range(0, conjunction_num):
            self.center_net_list[j].to(self.device)

    def get_entity_embedding(self, entity_ids: torch.Tensor, freeze=False, meta_parameters=None):
        if meta_parameters and 'entity_embeddings.weight' in meta_parameters:
    ...
    def get_projection_embedding(self, proj_ids: torch.Tensor, emb, net_index=0, freeze_rel=False, freeze_proj=False,
                                 meta_parameters=None):
        assert emb.shape[0] == len(proj_ids)
        net_index = max(0, net_index)
        if freeze_rel:
        ...
        use_proj_net = getattr(self, "projection_net_{}".format(net_index))
        if freeze_proj:
            pro_emb = use_proj_net(emb, rel_emb, True)
        else:
            if meta_parameters:
                pro_emb = use_proj_net.meta_forward(emb, rel_emb, meta_parameters)
            else:
                pro_emb = use_proj_net(emb, rel_emb, False)
        return pro_emb
    ...
    def get_conjunction_embedding(self, conj_emb: List[torch.Tensor], net_index=0, freeze=False, meta_parameters=None):
        net_index = max(0, net_index)
        use_center_net = getattr(self, "center_net_{}".format(net_index))
        all_emb = torch.stack(conj_emb)
        if freeze:
            freeze_center_net = torch.jit.freeze((torch.jit.script(use_center_net.eval())))
            emb = freeze_center_net(all_emb)
        else:
            if meta_parameters:
                emb = use_center_net.meta_forward(all_emb, meta_parameters)
            else:
                emb = use_center_net(all_emb)
        return emb
