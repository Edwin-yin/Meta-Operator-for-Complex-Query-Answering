...
class NLKEstimator(AppFOQEstimator):
    ...
    def get_projection_embedding(self, proj_ids: torch.Tensor, emb, freeze_rel=False, freeze_proj=False,
    ...
    def criterion(self, pred_emb: torch.Tensor, answer_set: List[IntList], freeze_entity=False, union: bool = False,
        ...
        positive_all_embedding = self.get_entity_embedding(torch.tensor(np.array(chosen_answer), device=self.device), 0,
                                                           **kwargs)
        neg_embedding = self.get_entity_embedding(
            torch.tensor(np.array(chosen_false_answer), device=self.device).view(-1), 0, **kwargs)
        neg_embedding = neg_embedding.view(-1, self.negative_size, 2 * self.entity_dim + self.group_number)
        # batch*n*dim
        if union:
    ...
    def compute_all_entity_logit(self, pred_emb: torch.Tensor, union: bool = False, **kwargs):
        all_entities = torch.LongTensor(range(self.n_entity)).to(self.device)
        all_embedding = self.get_entity_embedding(all_entities, 0, **kwargs)
