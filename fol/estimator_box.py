...
class BoxEstimator(AppFOQEstimator):
    ...
    def criterion(self, pred_emb: torch.Tensor, answer_set: List[IntList], freeze_entity=False, union=False,
                  meta_parameters=None):
        ...
        neg_embedding = self.get_entity_embedding(torch.tensor(np.array(chosen_false_answer),
                                                               device=self.device).view(-1), 0)
    ...
    def compute_all_entity_logit(self,
        ...
        all_embedding, _ = torch.chunk(
            self.get_entity_embedding(all_entities, 0), 2, dim=-1)
