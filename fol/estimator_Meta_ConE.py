...
class ConeProjection(MetaModule):
    def __init__(self, dim, hidden_dim, num_layers):
        ...
        self.layer1 = MyMetaLinear(self.entity_dim + self.relation_dim, self.hidden_dim)
        self.layer0 = MyMetaLinear(self.hidden_dim, self.entity_dim + self.relation_dim)
        for nl in range(2, num_layers + 1):
            setattr(self, "layer{}".format(nl), MyMetaLinear(self.hidden_dim, self.hidden_dim))
        for nl in range(num_layers + 1):
            nn.init.xavier_uniform_(getattr(self, "layer{}".format(nl)).weight)

    def forward(self, source_embedding_axis, source_embedding_arg, r_embedding_axis, r_embedding_arg, freeze=False,
        ...
        x = self.layer0(x, freeze=freeze, params=
            self.get_subdict(meta_parameters, 'projection_net_0.layer0') if meta_parameters else None)
...
class ConeIntersection(MetaModule):
    def __init__(self, dim, drop):
        super(ConeIntersection, self).__init__()
        self.dim = dim
        self.layer_axis1 = MyMetaLinear(self.dim * 2, self.dim)
        self.layer_arg1 = MyMetaLinear(self.dim * 2, self.dim)
        self.layer_axis2 = MyMetaLinear(self.dim, self.dim)
        self.layer_arg2 = MyMetaLinear(self.dim, self.dim)
    ...
    def forward(self, axis_embeddings, arg_embeddings, freeze=False, meta_parameters=None):
        logits = torch.cat([axis_embeddings - arg_embeddings, axis_embeddings + arg_embeddings], dim=-1)

        axis_layer1_act = F.relu(self.layer_axis1(logits, freeze=freeze, params=
            self.get_subdict(meta_parameters, 'center_net_0.layer_axis1') if meta_parameters else None))

        axis_attention = F.softmax(self.layer_axis2(axis_layer1_act, freeze=freeze,
         params=self.get_subdict(meta_parameters, 'center_net_0.layer_axis2') if meta_parameters else None), dim=0)
        ...
        arg_layer1_act = F.relu(self.layer_arg1(
            logits, freeze=freeze,
            params=self.get_subdict(meta_parameters, 'center_net_0.layer_arg1') if meta_parameters else None))
        arg_layer1_mean = torch.mean(arg_layer1_act, dim=0)
        gate = torch.sigmoid(self.layer_arg2(
            arg_layer1_mean, freeze=freeze,
            params=self.get_subdict(meta_parameters, 'center_net_0.layer_arg2') if meta_parameters else None))
...
class Meta_ConEstimator(AppFOQEstimator):
    name = "ConE"

    def __init__(self, n_entity, n_relation, entity_dim, relation_dim, hidden_dim, num_layer,
                 negative_sample_size, gamma, device, projection_num, conjunction_num,
                 center_reg=None, drop=0.):
        super(Meta_ConEstimator, self).__init__()
        ...
        self.relation_embeddings = self.relation_embedding = nn.Embedding(n_relation, self.relation_dim)
        nn.init.uniform_(
        ...
        self.projection_net_0 = ConeProjection(self.entity_dim, hidden_dim, num_layer)
        self.center_net_0 = ConeIntersection(self.entity_dim, drop)
    ...
    def get_projection_embedding(self, proj_ids: torch.Tensor, emb, net_index=-1, freeze_rel=False, freeze_proj=False,
        ...
        if net_index == -1:
            pro_axis_emb, pro_arg_emb = self.projection_net_0.forward(entity_axis_emb, entity_arg_emb, rel_axis_emb,
                                                                      rel_arg_emb, freeze_proj, meta_parameters)
        else:
            pro_axis_emb, pro_arg_emb = self.projection_net_0.forward(entity_axis_emb, entity_arg_emb, rel_axis_emb,
                                                                      rel_arg_emb, freeze_proj,
                                                                      meta_parameters[f'p_{net_index}'])
    ...
    def get_conjunction_embedding(self, conj_emb: List[torch.Tensor], net_index=-1, freeze=False, meta_parameters=None):
        ...
        if net_index == -1:
            new_axis_emb, new_arg_emb = self.center_net_0(stacked_axis_emb, stacked_arg_emb, freeze, meta_parameters)
        else:
            new_axis_emb, new_arg_emb = self.center_net_0(stacked_axis_emb, stacked_arg_emb, freeze,
                                                          meta_parameters[f'i_{net_index}'])
    ...
    def compute_logit(self, entity_emb, query_emb, meta_parameters=None):
        ...
        now_modulus = meta_parameters['modulus'] if meta_parameters else self.modulus
        logit = self.gamma - distance * now_modulus  # weird since modulus is a learnable parameter
    ...
    def criterion(self, pred_emb: torch.Tensor, answer_set: List[IntList], freeze_entity=False, union: bool = False,
        ...
        if union:
            positive_union_logit = self.compute_logit(answer_embedding.unsqueeze(1), pred_emb, meta_parameters)
            positive_logit = torch.max(positive_union_logit, dim=1)[0]
            negative_union_logit = self.compute_logit(neg_embedding.unsqueeze(1), pred_emb, meta_parameters)
            negative_logit = torch.max(negative_union_logit, dim=1)[0]
        else:
            positive_logit = self.compute_logit(answer_embedding, pred_emb, meta_parameters)
            negative_logit = self.compute_logit(neg_embedding, pred_emb, meta_parameters)  # b*negative
    ...
    def compute_all_entity_logit(self, pred_emb: torch.Tensor, union: bool = False, meta_parameters=None):
        ...
        for answer_part in chunk_of_answer:
            if union:
                union_part = self.compute_logit(answer_part.unsqueeze(0).unsqueeze(0), pred_emb, meta_parameters)
                logit_part = torch.max(union_part, dim=1)[0]
            else:
                logit_part = self.compute_logit(answer_part.unsqueeze(dim=0), pred_emb, meta_parameters)
