...
class BetaProjection(MetaModule):
    def __init__(self,
                 entity_dim,
                 relation_dim,
                 hidden_dim,
...
class BetaIntersection(MetaModule):
    def __init__(self, dim):
        super(BetaIntersection, self).__init__()
        self.dim = dim
        self.layer1 = MyMetaLinear(2 * self.dim, 2 * self.dim)
        self.layer2 = MyMetaLinear(2 * self.dim, self.dim)
        nn.init.xavier_uniform_(self.layer1.weight)
        nn.init.xavier_uniform_(self.layer2.weight)

    def forward(self, alpha_embeddings, beta_embeddings, freeze, meta_parameters):
        all_embeddings = torch.cat([alpha_embeddings, beta_embeddings], dim=-1)
        # (num_conj, batch_size, 2 * dim)
        layer1_act = F.relu(self.layer1(
            all_embeddings, freeze=freeze, params=self.get_subdict(meta_parameters, f'center_net_0.layer1')
            if meta_parameters else None))
        # (num_conj, batch_size, dim)
        attention = F.softmax(self.layer2(
            layer1_act, freeze=freeze, params=self.get_subdict(meta_parameters, f'center_net_0.layer2')
            if meta_parameters else None), dim=0)
        alpha_embedding = torch.sum(attention * alpha_embeddings, dim=0)
        beta_embedding = torch.sum(attention * beta_embeddings, dim=0)
        return alpha_embedding, beta_embedding


class Meta_BetaEstimator(AppFOQEstimator):
    name = "beta"

    def __init__(self, n_entity, n_relation, hidden_dim,
                 gamma, entity_dim, relation_dim, num_layers, projection_num, conjunction_num,
        ...
        self.center_net_0 = BetaIntersection(self.entity_dim)
        self.projection_net_0 = BetaProjection(self.entity_dim * 2,
                                             self.relation_dim,
                                             hidden_dim,
                                             self.projection_regularizer,
                                             num_layers)

    def get_entity_embedding(self, entity_ids: torch.LongTensor, freeze=False, meta_parameters=None):
        if freeze:
    ...
    def get_projection_embedding(self, proj_ids: torch.LongTensor, emb, net_symbol=-1, freeze_rel=False,
                                 freeze_proj=False, meta_parameters=None):
        ...
        if net_symbol == -1:
            ...
            pro_emb = self.projection_net_0.forward(emb, rel_emb, freeze_proj, meta_parameters[f'p_{net_symbol}'])
        return pro_emb

    def get_conjunction_embedding(self, conj_emb: List[torch.Tensor], net_symbol=-1, freeze=False, meta_parameters=None):
        ...
        if net_symbol == -1:
            new_alpha, new_beta = self.center_net_0.forward(all_alpha, all_beta, freeze, meta_parameters)
        else:
            new_alpha, new_beta = \
                self.center_net_0.forward(all_alpha, all_beta, freeze, meta_parameters[f'i_{net_symbol}'])
    ...
    def criterion(self, pred_emb: torch.Tensor, answer_set: List[IntList], freeze_entity=False, union: bool = False,
        ...
        answer_embedding = self.get_entity_embedding(torch.tensor(chosen_ans, device=self.device), freeze_entity,
                                                     meta_parameters)  # batch*1*dim
        all_neg_emb = self.get_entity_embedding(torch.tensor(
            chosen_false_ans, device=self.device).view(-1), freeze_entity, meta_parameters)
        # batch*negative*dim
        all_neg_emb = all_neg_emb.view(-1, self.negative_size, 2 * self.entity_dim)
        if union:
    ...
    def compute_all_entity_logit(self, pred_emb: torch.Tensor, union: bool = False,
                                 meta_parameters=None) -> torch.Tensor:
...
class BetaEstimator4V(AppFOQEstimator):
    ...
    def get_projection_embedding(self, proj_ids: torch.LongTensor, emb, freeze_rel=False, freeze_proj=False,
                                 meta_parameters=None):
        assert emb.shape[0] == len(proj_ids)
    ...
    def get_conjunction_embedding(self, conj_emb: List[torch.Tensor], freeze=False, meta_parameters=None):
        sub_alpha_list, sub_beta_list = [], []
        for sub_emb in conj_emb:
            sub_alpha, sub_beta = torch.chunk(sub_emb, 2, dim=-1)
            sub_alpha_list.append(sub_alpha)  # b*dim
    ...
    def criterion(self, pred_emb: torch.Tensor, answer_set: List[IntList], freeze_entity=False, union: bool = False,
        ...
        if union:
            ...
            positive_logit = self.compute_logit(answer_embedding, query_dist)
        all_neg_emb = self.get_entity_embedding(torch.tensor(
            np.array(chosen_false_ans), device=self.device).view(-1), 0)
        # batch*negative*dim
        ...
        return positive_logit.unsqueeze(1), negative_logit, subsampling_weight.to(
                                                                self.device)
    ...
    def compute_all_entity_logit(self, pred_emb: torch.Tensor, union: bool = False,
                                 meta_parameters=None) -> torch.Tensor:
        all_entities = torch.LongTensor(range(self.n_entity)).to(self.device)
        all_embedding = self.get_entity_embedding(all_entities, 0)  # nentity*dim
        pred_alpha, pred_beta = torch.chunk(
            pred_emb, 2, dim=-1)  # batch*(disj)*dim
        query_dist = torch.distributions.beta.Beta(
            pred_alpha.unsqueeze(-2), pred_beta.unsqueeze(-2))
