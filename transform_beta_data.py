...
def align_with_whole_formulas(store_fold, partial_formula_file, whole_formula_file, prefix):
    partial_formula_data = pd.read_csv(join(store_fold, partial_formula_file))
    whole_formula_data = pd.read_csv(join(store_fold, whole_formula_file))
    old2new_dict = {}
    for i, original_formula in enumerate(partial_formula_data['original']):
        whole_original_formula = whole_formula_data['original']
        now_line = whole_original_formula[whole_original_formula == original_formula]
        real_id = whole_formula_data.loc[now_line.index[0], 'formula_id']
        old2new_dict[partial_formula_data.loc[i, 'formula_id']] = real_id
        partial_formula_data.loc[i, 'formula_id'] = real_id

    for old_type_str in old2new_dict:
        new_type_str = old2new_dict[old_type_str]
        if new_type_str != old_type_str:
            os.rename(join(store_fold, f"{prefix}-{old_type_str}.csv"),
                      join(store_fold, f"{prefix}-{new_type_str}-tmp.csv"))
    for old_type_str in old2new_dict:
        new_type_str = old2new_dict[old_type_str]
        if new_type_str != old_type_str:
            os.rename(join(store_fold, f"{prefix}-{new_type_str}-tmp.csv"),
                      join(store_fold, f"{prefix}-{new_type_str}.csv"))
    partial_formula_data.to_csv(join(store_fold, partial_formula_file))
...
if __name__ == "__main__":
    ...
    for data_path in beta_data_folders:
        ...
        store_json_query_benchmark_form_with_check(test_queries,
                                                   ...
                                                   beta_names=None)
        '''
        align_with_whole_formulas(data_path, 'train_formulas.csv', 'test_formulas.csv', 'train')
        align_with_whole_formulas(data_path, 'valid_formulas.csv', 'test_formulas.csv', 'valid')
